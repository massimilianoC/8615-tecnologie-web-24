...
class DatabaseHelper{
    ...
    public function getActiveFollowersByUserId($id){
        $query = "select u.* from users u
        left join followers f on  u.idUSER = f.fkFollower 
    ...
    public function getActiveFollowedByUserId($id){
        $query = "select u.* from users u
    ...
    public function updateNotification( $dataVisualizzazione, $dataArchiviazione, $read, $idNOTIFICATION){
        if($dataArchiviazione==0){
            ...
            $stmt = $this->db->prepare($query);
            $stmt->bind_param('ssii',$dataVisualizzazione, $dataArchiviazione, $read, $idNOTIFICATION);
        }
