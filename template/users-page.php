    ...
    <ul class="list-group">
        <?php foreach($template_data["users"] as $utente): ?>
            <li class="list-group-item d-flex justify-content-between align-items-center user list item">
                ...
                <span><img class="user profile" src="<?php echo urlencode($userImg); ?>" alt="" /></span>
                ...
                <?php if($mySelf ==0) : ?>
                    ...
                    <?php endforeach; ?>
                        <form id="form-follow-<?php echo $utente["idUSER"]?>" action="#" method="POST">
                            ...
                            <button id="<?php echo $utente["idUSER"]?>" class="follow-button btn-sm btn btn-<?php if($amIFollowing==1) echo 'outline-secondary'; else echo 'primary' ?>"  type="button">
