          ...
          <ul class="list-group">
              <?php if($notification['fkFollow']!= NULL): ?>
                ...
                <li class="list-group-item alert alert-warning alert-dismissible fade show <?php if($notification['read'] > 0) echo ' read ' ; echo 'list-group-item-warning ' ?>" >
                  <span><em class="bi bi-person-add"></em> <a href="index.php?page=userprofile&iduser=<?php echo $follower['idUSER']?>"><strong><?php echo $follower['nome'].' '.$follower['cognome']  ?></strong></a> ha iniziato a seguirti</span>
              <?php elseif($notification['fkPost']!= NULL): ?>
                ...
                <li class="list-group-item alert alert-warning alert-dismissible fade show <?php if($notification['read'] > 0) echo ' read' ; echo ($post['isComment']==0)? ' list-group-item-info ' : ' list-group-item-primary ' ?>" >
                  <?php if($post['isComment']==0): ?>
                    <span><em class="bi bi-file-post"></em><a href="index.php?page=userprofile&iduser=<?php echo $user['idUSER']?>"><strong><?php echo $user['nome'].' '.$user['cognome'] ?></strong></a> ha pubblicato un <a href="index.php#<?php echo $post['fkParent'] ?>?page=home">post</a></span>
                  <?php else: ?>
                    <span><em class="bi bi-chat-dots-fill"></em> 
                  ...
                  <?php endif; ?>
                <?php endif; ?>
                <button id="notification<?php echo $notification['idNOTIFICATION'] ?>" type="button" class="archive notification btn btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
              </li>
          </ul>
        <?php endforeach; ?>
